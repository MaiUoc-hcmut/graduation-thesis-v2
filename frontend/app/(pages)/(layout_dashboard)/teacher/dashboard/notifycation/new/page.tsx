...
export default function CreateNotify() {
    ...
    useEffect(() => {
        async function fetchData() {
            if (currentCourse)
                await courseApi.getAllStudenBuyCourseOfTeacher(currentCourse, `${user.id}`, page).then((data: any) => {
    ...
    return (
        <div>
            ...
            <div className='flex  items-center'>
                ...
                {/* <div className={`${currentCourse ? '' : 'hidden'} w-1/3`}>
                    <select id="courses" name="id_student" className="w-full bg-white border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" >
                        <option value="" defaultChecked>Chọn học sinh</option>
                ...
                <div className={`${currentCourse ? '' : 'hidden'} w-full`}>
                    <button
            ...
            <div className='w-3/4 flex flex-col mt-10'>
                <form onSubmit={handleSubmit(async (data: any) => {
                    const studentIds = Object.keys(listStudent) || listStudent?.map((student: any) => student.id);
                    const formData = {
                        data: {
                    ...
                    await notifyApi.teacherSendNotify(formData).then((data: any) => { }).catch((err: any) => { })
                    setChange(!change)
                    reset()
                })}>
                    <textarea
